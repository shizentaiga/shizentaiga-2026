...
Cloudflare エコシステムによるフルスタックな内製化を実現するプロジェクト。
...
本プロジェクトの設計思想・仕様・運用手順を以下の4レイヤーで集約管理しています。

* **戦略・コアロジック(00番台)**: プロジェクトの目的、ロードマップ、および Grid Inventory Model に基づく予約ステート管理の定義。
* **システム設計・規約(10番台)**: 技術スタック、D1データベース構造（ULID規約）、SQL実装および Vitest によるテスト手順、UIコンポーネント構成。
* **実装・構成定義(20番台)**: バリデーションや DOM 操作などの共通実装規約、および各種設定ファイル（package.json / tsconfig 等）の意図。
* **外部連携・運用(30番台)**: Stripe / PayPal 連携と自己修復ロジック、緊急時の Git 切り戻し手順、および環境構築・デプロイの全履歴。
