...
- [x] Day 1: D1スキーマ刷新(v2.6)（description追加、外部キー制約設定）
- [x] Day 2: マスタデータ投入（seed_01による店舗・プラン投入完了）
- [ ] Day 3: プラン一覧(/services)本番実装（test10のロジックを正規画面へ統合）

#### 🧠 Step B：予約エンジン・コアロジック
- [ ] Day 4: date-fns 導入（JST/UTC管理とカレンダー生成語彙の統一）
