...
* **戦略・コアロジック**: プロジェクトの目的、ロードマップ、および Grid Inventory Model に基づく予約ステート管理の定義。
* **システム設計・規約**: 技術スタック、D1データベース構造（ULID規約）、SQL実装および Vitest によるテスト手順、UIコンポーネント構成。
* **実装・構成定義**: バリデーションや DOM 操作などの共通実装規約、および各種設定ファイル（package.json / tsconfig 等）の意図。
* **外部連携・運用**: Stripe / PayPal 連携と自己修復ロジック、緊急時の Git 切り戻し手順、および環境構築・デプロイの全履歴。
